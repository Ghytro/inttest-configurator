...
class UserEditPage extends Component<any, UserListModel> {
          ...
          .then((permResp: any) => {
            ...
            this.roleById.forEach((v, k) => {
              roleListTableModel.push({
                ...
                name: v.name,
                desc: v.desc,
                permissions: v.perm_ids?.map(
                  (permId) => this.permById.get(permId)?.name
                ),
              });
            });
            ...
            this.userApi
              ...
              .then(({ data }) => {
                this.setState({
                  userListTableModel: data.map((item) => ({
                    id: item.id,
                    username: item.username,
                    roleNames: item.role_ids?.map(
                      (roleId) => this.roleById.get(roleId)?.name
                    ),
                  })),
                });
              })
  ...
  deleteRole(roleId: number) {
    this.userApi.deleteRole(roleId).then(({ data }) => {
      this.fetchUserListTableData();
    });
  }
  ...
  createUser() {
    ...
    this.userApi
      .createUser({
        ...
        role_ids: this.state.userCreateRolesCheckboxValues?.map(
          (roleName) => roles.find((role) => role.name == roleName)?.id!
        ),
      })
      .then(({ data }) => {
        this.setState({
  ...
  createRole() {
    ...
    this.userApi
      .createRole({
        ...
        perm_ids: this.state.roleCreatePermsCheckboxValues?.map(
          (permName) => perms.find((x) => x.name == permName)?.id!
        ),
      })
      .then(({ data }) => {
        this.setState({
  ...
  render(): React.ReactNode {
      ...
      },
      {
        ...
        render: (_, user) => (
          <Button
            ...
            icon={<EditOutlined />}
            onClick={() => {
              const allCheckboxValues = this.state.roleListTableModel?.map(
                (x) => x.name
              );
              this.setState({
    ...
    const roleListTableCols = [
      ...
      },
      {
        ...
        render: (_, record) =>
          record.permissions.map((permName) => (
            <>
              <span>{permName}</span>
              <br />
            </>
          ...
          <Button
            ...
            onClick={() => {
              const allCheckboxValues =
                this.state.permissionListTableModel?.map((x) => x.name);
              this.setState({
      ...
      <>
        ...
        <Modal
          ...
          onOk={() => {
            ...
            this.updateUser(
              ...
              this.state.userRolesEditCheckboxValues!.map(
                (item) => allRoles.find((v) => v.name == item)?.id!
              )
            );
          }}
          onCancel={() =>
          ...
          <Form layout="vertical">
            <Form.Item label="Логин">
              <Input
                ...
                onChange={(newValue) =>
                  this.setState({
                    usernameEditInputValue: newValue.target.value,
                  })
                }
              />
            </Form.Item>
            <Form.Item label="Роли">
              <Checkbox.Group
                ...
                onChange={(newValue) => {
                  ...
                  this.setState({
                    userRolesEditCheckboxValues: newValue.map((v) =>
                      v.toString()
                    ),
                  });
                }}
        ...
        <Modal
          ...
          onOk={() => {
            ...
            const permIds = this.state.editedRolePermsCheckboxValues!.map(
              (permName) => allPerms.find((p) => p.name == permName)?.id!
            );
            this.updateRole(
              this.state.editedRoleId!,
              this.state.editedRoleNameInputValue!,
              this.state.editedRoleDescInputValue,
              permIds
            );
          }}
          ...
          <Form layout="vertical">
            <Form.Item label="Наименование">
              <Input
                ...
                onChange={(e) =>
                  this.setState({ editedRoleNameInputValue: e.target.value })
                }
              />
            </Form.Item>
            <Form.Item label="Описание">
              <Input
                ...
                onChange={(e) =>
                  this.setState({ editedRoleDescInputValue: e.target.value })
                }
              />
            </Form.Item>
            <Form.Item label="Разрешения">
              <Checkbox.Group
                ...
                onChange={(newValue) => {
                  console.log("onChange triggered");
                  this.setState({
                    editedRolePermsCheckboxValues: newValue.map((v) =>
                      v.toString()
                    ),
                  });
                }}
        ...
        <FloatButton
          ...
          type="primary"
          onClick={() => {
            const allRolesCheckboxes = this.state.roleListTableModel?.map(
              (x) => x.name!
            );
            this.setState({
              userCreateDialogOpened: true,
              userCreateRolesCheckboxAllValues: allRolesCheckboxes,
            });
          }}
          style={{ right: 24 }}
        />
        <FloatButton
          ...
          type="default"
          onClick={() => {
            const allPermsCheckboxes = this.state.permissionListTableModel?.map(
              (x) => x.name!
            );
            this.setState({
              roleCreateDialogOpened: true,
              roleCreatePermsCheckboxAllValues: allPermsCheckboxes,
            });
          }}
          style={{ right: 94 }}
        />
        ...
        >
          <Form layout="vertical">
            <Form.Item label="Наименование">
              <Input
                onChange={(e) =>
                  this.setState({ roleCreateNameInputValue: e.target.value })
                }
              />
            </Form.Item>
            <Form.Item label="Описание">
              <Input
                onChange={(e) =>
                  this.setState({ roleCreateDescInputValue: e.target.value })
                }
              />
            </Form.Item>
            <Form.Item label="Разрешения">
              <Checkbox.Group
                ...
                onChange={(values) =>
                  this.setState({
                    roleCreatePermsCheckboxValues: values.map((x) =>
                      x.toString()
                    ),
                  })
                }
        ...
        >
          <Form layout="vertical">
            <Form.Item label="Логин">
              <Input
                onChange={(e) =>
                  this.setState({ userCreateLoginInputValue: e.target.value })
                }
              />
            </Form.Item>
            <Form.Item label="Пароль">
              <Input
                ...
                onChange={(e) =>
                  this.setState({
                    userCreatePasswordInputValue: e.target.value,
                  })
                }
              />
            </Form.Item>
            <Form.Item label="Роли">
              <Checkbox.Group
                ...
                onChange={(values) =>
                  this.setState({
                    userCreateRolesCheckboxValues: values.map((x) =>
                      x.toString()
                    ),
                  })
                }
